...
class SettingsWindow:
    ...
    def _create_widgets(self):
        ...
        # タイトル
        title_label = ttk.Label(main_frame, text="タスク設定", 
                               font=("Arial", 14, "bold"))
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 10))
        
        # タスク一覧
        ttk.Label(main_frame, text="登録済みタスク:").grid(row=1, column=0, sticky=tk.W, pady=(0, 5))
        
        # ツリービュー
        columns = ("時間", "タスク名", "状態")
        self.task_tree = ttk.Treeview(main_frame, columns=columns, show="headings", height=10)
        ...
        self.task_tree.grid(row=2, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
    ...
    def _edit_task(self):
        ...
        if dialog.result:
            time_str, task_names, enabled = dialog.result
            self.task_manager.config.update_task(task["id"], time_str, task_names, enabled)
            self._load_tasks()
...
class TaskDialog:
    ...
    def _create_widgets(self, task: dict = None):
        ...
        # 時刻設定
        ttk.Label(main_frame, text="時刻 (HH:MM):").grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        ...
        # タスク名設定
        ttk.Label(main_frame, text="タスク名 (複数設定可):").grid(row=1, column=0, sticky=(tk.W, tk.N), pady=(10, 5))
        ...
        self.task_listbox.grid(row=0, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        task_scrollbar = ttk.Scrollbar(task_frame, orient=tk.VERTICAL, command=self.task_listbox.yview)
        ...
        add_task_button = ttk.Button(task_frame, text="追加", command=self._add_task_name)
        add_task_button.grid(row=1, column=1, padx=(5, 0), pady=(5, 0))
        
        remove_task_button = ttk.Button(task_frame, text="削除", command=self._remove_task_name)
        remove_task_button.grid(row=1, column=2, padx=(5, 0), pady=(5, 0))
        
        # 有効/無効設定
        self.enabled_var = tk.BooleanVar(value=True)
        enabled_check = ttk.Checkbutton(main_frame, text="タスクを有効にする", 
        ...
        cancel_button = ttk.Button(button_frame, text="キャンセル", command=self._cancel_clicked)
    ...
    def _ok_clicked(self):
        ...
        if not task_names:
            messagebox.showerror("エラー", "少なくとも1つのタスク名を入力してください。")
            return
