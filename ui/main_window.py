...
class MainWindow:
    ...
    def _create_widgets(self):
        ...
        status_label = ttk.Label(
            main_frame, text="監視中", foreground="green", font=("Arial", 10)
        )
        status_label.grid(row=1, column=0, columnspan=2, pady=(0, 10), sticky="")

        # ボタン
        settings_button = ttk.Button(
            main_frame, text="タスク設定", command=self._open_settings
        )
        settings_button.grid(row=2, column=0, padx=(0, 5), sticky=(tk.W, tk.E))

        log_button = ttk.Button(
            main_frame, text="ログ・達成率", command=self._open_log_window
        )
        log_button.grid(row=2, column=1, padx=(5, 0), sticky=(tk.W, tk.E))

        # 終了ボタン
        exit_button = ttk.Button(
            main_frame, text="終了", command=self._exit_application
        )
        exit_button.grid(
            row=3, column=0, columnspan=2, pady=(10, 0), sticky=(tk.W, tk.E)
        )
