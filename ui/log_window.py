...
class LogWindow:
    ...
    def _create_widgets(self):
        ...
        title_label = ttk.Label(
            main_frame, text="ログ・達成率", font=("Arial", 14, "bold")
        )
        ...
        year_combo = ttk.Combobox(
            month_frame, textvariable=self.year_var, width=8, state="readonly"
        )
        year_combo["values"] = [
            str(year) for year in range(self.current_year - 1, self.current_year + 2)
        ]
        ...
        month_combo = ttk.Combobox(
            month_frame, textvariable=self.month_var, width=8, state="readonly"
        )
        ...
        update_button = ttk.Button(
            month_frame, text="更新", command=self._update_display
        )
        update_button.pack(side=tk.LEFT, padx=(0, 10))

        # 週末除外チェックボックス
        self.exclude_weekends_var = tk.BooleanVar(
            value=self.task_manager.config.get_exclude_weekends()
        )
    ...
    def _create_achievement_tab(self):
        ...
        self.achievement_label = ttk.Label(
            stats_frame, text="", font=("Arial", 16, "bold")
        )
        ...
        ttk.Label(
            calendar_frame, text="カレンダー表示", font=("Arial", 12, "bold")
        ).pack(anchor=tk.W, pady=(0, 10))

        self.calendar_text = tk.Text(calendar_frame, height=25, width=60)
        calendar_scrollbar = ttk.Scrollbar(
            calendar_frame, orient=tk.VERTICAL, command=self.calendar_text.yview
        )
    ...
    def _create_target_days_tab(self):
        ...
        ttk.Label(settings_frame, text="対象日設定", font=("Arial", 12, "bold")).pack(
        ...
        ttk.Button(
            preset_frame, text="平日のみ選択", command=self._select_weekdays_only
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(
            preset_frame,
            text="土日除外をリセット",
            command=self._reset_weekend_exclusion,
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(preset_frame, text="全日選択", command=self._select_all_days).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(preset_frame, text="全日解除", command=self._unselect_all_days).pack(
            side=tk.LEFT
        )
    ...
    def _update_achievement_tab(self, year: int, month: int):
        ...
        for day in range(1, last_day + 1):
            ...
            if not self.task_manager.config.is_date_included(date_obj):
                # 対象外の日はスキップ
                daily_stats[day] = {"total": 0, "completed": 0, "rate": 0}
                continue
        ...
        for day in range(1, last_day + 1):
            ...
            calendar_text += (
                f"{day:2d}日: {status} ({stats['completed']}/{stats['total']})\n"
            )
    ...
    def _update_graph_tab(self):
        ...
        for i in range(12):
            ...
            for day in range(1, last_day + 1):
                ...
                for task in enabled_tasks:
                    # タスクの登録日を取得
                    task_created_date = self.task_manager.config.get_task_created_date(
                        task
                    )
                    task_created = datetime.strptime(
                        task_created_date, "%Y-%m-%d"
                    ).date()

                    # タスクが登録された日以降、かつ今日以前の場合のみカウント
                    # さらに、is_date_includedで対象日かどうかをチェック
                    if (
                        task_created <= date_obj <= today
                        and self.task_manager.config.is_date_included(date_obj)
                    ):
    ...
    def _update_detail_tab(self, year: int, month: int):
        ...
        for log in logs:
            ...
            self.log_tree.insert(
                "", "end", values=(log["date"], log["time"], log["task_name"], status)
            )
