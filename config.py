...
class Config:
    def __init__(self):
        ...
        self.calendar_overrides_file = os.path.join(self.data_dir, "calendar_overrides.json")
    ...
    def _save_calendar_overrides(self, overrides: Dict[str, Dict[str, bool]]):
    ...
    def load_calendar_overrides(self) -> Dict[str, Dict[str, bool]]:
        ...
        try:
            with open(self.calendar_overrides_file, 'r', encoding='utf-8') as f:
                return json.load(f)
        except (FileNotFoundError, json.JSONDecodeError):
            return {}
    
    def save_calendar_overrides(self, overrides: Dict[str, Dict[str, bool]]):
        """カレンダーオーバーライドデータを保存"""
    ...
    def set_day_override(self, year: int, month: int, day: int, included: bool):
        """特定日のオーバーライド設定を保存"""
        overrides = self.load_calendar_overrides()
        month_key = f"{year:04d}-{month:02d}"
    ...
    def clear_month_overrides(self, year: int, month: int):
        """指定月のオーバーライド設定をクリア"""
        overrides = self.load_calendar_overrides()
        month_key = f"{year:04d}-{month:02d}"
